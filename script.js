...
function updateTimerUI(t){
  ...
  if(!el) return;
  const isLow = t <= 10;
  /* Only update text if changed (avoids layout thrash) */
  const display = String(Math.floor(t/60)).padStart(2,'0') + ':' + String(t%60).padStart(2,'0');
  if(el.textContent !== display) el.textContent = display;

  /* Casino gold timer styling */
  el.style.color = isLow ? '#ff3355' : '#ffd700';
  el.style.textShadow = isLow
    ? '0 0 40px #ff3355,0 0 80px rgba(255,51,85,.5),2px 2px 0 #6a0015'
    : '0 0 30px #ffd700,0 0 60px rgba(200,140,0,.7),0 0 100px rgba(160,100,0,.3),2px 2px 0 #5a3a00';

  if(!GAME.resultAnimating){
    const area = document.getElementById('casinoBetArea');
    const lock = document.getElementById('casinoLock');
    if(area){ area.style.opacity=isLow?'.28':'1'; area.style.pointerEvents=isLow?'none':'auto'; }
    if(lock) lock.style.display=isLow?'block':'none';
  }
...
function renderGames(){
  ...
  /* chip helpers */
  function chipBg(n){
    if(n===0||n===5) return 'radial-gradient(circle at 38% 28%,rgba(255,255,255,.32),rgba(168,85,247,.65) 42%,rgba(80,15,160,.98))';
    if(n%2===0)      return 'radial-gradient(circle at 38% 28%,rgba(255,255,255,.32),rgba(239,68,68,.65) 42%,rgba(130,10,10,.98))';
    return                 'radial-gradient(circle at 38% 28%,rgba(255,255,255,.32),rgba(34,197,94,.65) 42%,rgba(8,90,30,.98))';
  }
  function chipShadow(n){
    const g=n===0||n===5?'168,85,247':n%2===0?'239,68,68':'34,197,94';
    return `0 6px 22px rgba(0,0,0,.8),0 0 0 3px rgba(${g},.85),0 0 20px rgba(${g},.4),inset 0 3px 6px rgba(255,255,255,.3),inset 0 -4px 8px rgba(0,0,0,.55)`;
  }
  ...
  <div id="casinoGameBg">
    ...
    <!-- Spotlights -->
    <div class="casino-spotlight" style="left:18%"></div>
    <div class="casino-spotlight" style="right:12%;animation-delay:3.5s;animation-direction:reverse"></div>

    <!-- Gold particles -->
    <div class="casino-particle" style="left:5%;animation-duration:6s;animation-delay:0s"></div>
    <div class="casino-particle" style="left:18%;animation-duration:8s;animation-delay:1s;width:3px;height:3px"></div>
    <div class="casino-particle" style="left:35%;animation-duration:5s;animation-delay:2.2s"></div>
    <div class="casino-particle" style="left:52%;animation-duration:9s;animation-delay:.5s;width:5px;height:5px"></div>
    <div class="casino-particle" style="left:68%;animation-duration:7s;animation-delay:1.8s"></div>
    <div class="casino-particle" style="left:80%;animation-duration:6.5s;animation-delay:.8s;width:3px;height:3px"></div>
    <div class="casino-particle" style="left:90%;animation-duration:8.5s;animation-delay:2.8s"></div>

    <!-- TOP ROW -->
    <div id="casinoTopRow">
      <div class="casino-period-block">
        <div class="lbl">PERIOD</div>
        <div class="val">${fmtPeriod(cur)}</div>
      </div>
      <div class="casino-live-badge">
        <div class="casino-live-dot"></div>LIVE
      </div>
      <div style="display:flex;gap:5px;align-items:center">
        ${GAME.history.slice(0,5).map(h=>{
          const bg=COL_HEX[h.colour]||'#888';
          return `<div class="casino-hist-chip" style="background:radial-gradient(circle at 38% 28%,rgba(255,255,255,.28),${bg}cc 42%,${bg}ff)">${h.num}</div>`;
        }).join('')}
      </div>
    </div>

    <!-- TIMER -->
    <div id="casinoTimerSection">
      <div class="casino-time-lbl">TIME REMAINING</div>
      <div id="gameTimerNum" style="${isLow?'color:#ff4060;animation:goldPulse .5s ease-in-out infinite,timerFlicker 1.5s ease-in-out infinite;':''}">${mm}:${ss}</div>
    </div>

    <!-- BET AREA -->
    <div id="casinoBetArea" style="opacity:${isLow?.28:1};pointer-events:${isLow?'none':'auto'};transition:opacity .3s">

      <!-- Active bets pill bar -->
      ${GAME.bets.length>0?`<div id="casinoBetSummary">
        <div style="font-size:9px;color:rgba(212,168,60,.65);letter-spacing:1.5px;margin-bottom:5px;font-family:'Oswald',sans-serif">${GAME.bets.length} BET${GAME.bets.length>1?'S':''} ACTIVE · ₹${totalBetAmt} TOTAL</div>
        <div>${GAME.bets.map(b=>`<span class="cs-bet-pill" style="background:rgba(212,168,60,.1);border-color:rgba(212,168,60,.4);color:rgba(212,168,60,.9);font-family:'Oswald',sans-serif;font-size:12px;font-weight:700">${b.type==='number'?'No.'+b.num:b.side.toUpperCase()} ₹${b.amt}</span>`).join('')}</div>
      </div>`:''}

      <!-- GREEN / VIOLET / RED -->
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:9px;margin-bottom:10px">
        <button class="casino-colour-btn" onclick="placeBetGame('small')"
          style="box-shadow:0 8px 28px rgba(0,0,0,.75),0 0 20px rgba(34,197,94,.2),inset 0 1px 0 rgba(255,255,255,.18),inset 0 -3px 0 rgba(0,0,0,.4);border:3px solid rgba(34,197,94,.85)">
          <div class="casino-colour-btn-inner" style="background:linear-gradient(165deg,rgba(34,197,94,.45),rgba(8,90,30,.98))">
            <div class="casino-colour-btn-name">Green</div>
            <div class="casino-colour-btn-odds">1.9×</div>
          </div>
        </button>
        <button class="casino-colour-btn" onclick="placeBetGame('small')"
          style="box-shadow:0 8px 28px rgba(0,0,0,.75),0 0 20px rgba(168,85,247,.2),inset 0 1px 0 rgba(255,255,255,.18),inset 0 -3px 0 rgba(0,0,0,.4);border:3px solid rgba(168,85,247,.85)">
          <div class="casino-colour-btn-inner" style="background:linear-gradient(165deg,rgba(168,85,247,.45),rgba(80,15,160,.98))">
            <div class="casino-colour-btn-name">Violet</div>
            <div class="casino-colour-btn-odds">1.9×</div>
          </div>
        </button>
        <button class="casino-colour-btn" onclick="placeBetGame('big')"
          style="box-shadow:0 8px 28px rgba(0,0,0,.75),0 0 20px rgba(239,68,68,.2),inset 0 1px 0 rgba(255,255,255,.18),inset 0 -3px 0 rgba(0,0,0,.4);border:3px solid rgba(239,68,68,.85)">
          <div class="casino-colour-btn-inner" style="background:linear-gradient(165deg,rgba(239,68,68,.45),rgba(130,10,10,.98))">
            <div class="casino-colour-btn-name">Red</div>
            <div class="casino-colour-btn-odds">1.9×</div>
          </div>
        </button>
      </div>
      ...
      <!-- NUMBER CHIP GRID -->
      <div class="casino-chip-grid-wrap">
        <div class="casino-chip-label">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="rgba(212,168,60,.8)" stroke-width="2.5"/><circle cx="12" cy="12" r="3.5" fill="rgba(212,168,60,.6)"/></svg>
          NUMBER BET — EXACT MATCH = 8× PAYOUT
        </div>
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px">
          ${[0,1,5,7,3,2,4,6,8,9].map(n=>`
            <button class="casino-number-chip"
              onclick="placeBetOnNumber(${n})"
              style="background:${chipBg(n)};box-shadow:${chipShadow(n)}">
              ${n}
            </button>`).join('')}
        </div>
      </div>
      ...
      <div style="margin-bottom:10px">
        <div style="font-size:9px;color:rgba(212,168,60,.5);font-weight:700;letter-spacing:2px;margin-bottom:8px">BET AMOUNT [tap to change]</div>
        <div style="display:flex;gap:7px;flex-wrap:wrap">
          ${[10,50,100,500,1000].map(a=>`<button class="casino-amt-chip ${GAME.betAmt===a?'active':''}" data-amt="${a}"
            onclick="GAME.betAmt=${a};document.querySelectorAll('.casino-amt-chip').forEach(c=>c.classList.toggle('active',parseInt(c.dataset.amt)===${a}))">
            ₹${a>=1000?'1K':a}</button>`).join('')}
        </div>
      </div>

      <!-- SMALL / BIG -->
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:6px">
        <button class="casino-sb-btn" onclick="placeBetGame('small')"
          style="background:linear-gradient(165deg,rgba(40,80,200,.5),rgba(8,25,120,.98));border:3px solid rgba(100,150,255,.85);box-shadow:0 10px 32px rgba(0,0,0,.8),0 0 24px rgba(96,165,250,.18),inset 0 1px 0 rgba(255,255,255,.18),inset 0 -3px 0 rgba(0,0,0,.5);color:#8abaff">
          <span class="casino-sb-btn-name">SMALL</span>
          <span class="casino-sb-btn-odds">0–5 · 1.9×</span>
        </button>
        <button class="casino-sb-btn" onclick="placeBetGame('big')"
          style="background:linear-gradient(165deg,rgba(180,30,30,.5),rgba(110,8,8,.98));border:3px solid rgba(239,68,68,.85);box-shadow:0 10px 32px rgba(0,0,0,.8),0 0 24px rgba(239,68,68,.18),inset 0 1px 0 rgba(255,255,255,.18),inset 0 -3px 0 rgba(0,0,0,.5);color:#ff8888">
          <span class="casino-sb-btn-name">BIG</span>
          <span class="casino-sb-btn-odds">6–9 · 1.9×</span>
        </button>
      </div>

    </div><!-- /casinoBetArea -->

    ${isLow?`<div id="casinoLock">🔒 BETTING CLOSED — LAST 10 SECONDS</div>`:''}

    <!-- LEDGER TRIGGER -->
    <div id="betLedgerTrigger" onclick="toggleLedger()" role="button" style="margin:12px 12px 0">
    ...
    <!-- LAST 20 ROUNDS -->
    <div style="margin:12px 12px 110px;background:rgba(0,0,0,.42);border:1.5px solid rgba(212,168,60,.18);border-radius:18px;padding:14px;backdrop-filter:blur(10px)">
      <div style="font-size:10px;color:rgba(212,168,60,.45);font-weight:700;letter-spacing:1.5px;margin-bottom:12px">LAST 20 ROUNDS · SAME FOR ALL USERS</div>
      ${GAME.history.length===0
        ?`<div style="text-align:center;color:rgba(212,168,60,.28);padding:24px;font-size:13px;font-family:'Oswald',sans-serif">Waiting for first round…</div>`
        :`<div style="display:grid;grid-template-columns:auto 1fr auto auto;gap:10px;font-size:9px;color:rgba(212,168,60,.28);font-weight:700;letter-spacing:.8px;padding:0 4px 8px;border-bottom:1px solid rgba(212,168,60,.1);margin-bottom:4px"><span>#</span><span>PERIOD</span><span style="text-align:center">NO.</span><span style="text-align:right">RESULT</span></div>`+
        GAME.history.slice(0,20).map((h,i)=>`
          <div style="display:grid;grid-template-columns:auto 1fr auto auto;gap:10px;align-items:center;padding:8px 4px;border-bottom:1px solid rgba(212,168,60,.07)">
            <div style="font-size:10px;color:rgba(212,168,60,.22);font-family:'Oswald',sans-serif">${i+1}</div>
            <div style="font-size:11px;color:rgba(212,168,60,.4);font-family:'Oswald',sans-serif">${h.period}</div>
            <div style="width:30px;height:30px;border-radius:50%;background:${COL_HEX[h.colour]};border:2px solid rgba(212,168,60,.55);display:flex;align-items:center;justify-content:center;font-family:'Oswald',sans-serif;font-weight:700;font-size:14px;color:#fff;box-shadow:0 0 8px ${COL_HEX[h.colour]}55">${h.num}</div>
